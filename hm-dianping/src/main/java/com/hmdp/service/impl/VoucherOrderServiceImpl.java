...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    @PostConstruct
    public void init(){
        log.info("voucherServiceImpl加载完成之后，执行异步线程任务");
     ...
     * 1、执行命令XREADGROUP GROUP group_name consumer_name count block stream key_name 0
     * 表示从pendingList中获取没有确认的消息
     * 2、如果消息为空，那么直接退出循环
     * 3、消息不为空，那么解析消息，生成秒杀订单
     * 4、发送确认
     * 5、如果在1-4的过程中，发生了异常，那么需要等待一小段时间，然后在重试
     */
